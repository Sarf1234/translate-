...
const leafVariants = {
  hidden: { opacity: 0, x: 100 }, // animate from right to left
  visible: (i) => ({
    opacity: 1,
    x: 0,
    transition: {
  ...
  return (
    ...
    >
      {Array.from({ length: totalPairs }).map((_, i) => {
        ...
        const animationIndex = i; // animate from left-most pair to right (reverse order visually = right to left)

        return (
          <>
            {isStatic ? (
              <>
                <path key={`static-top-${i}`} d={topRow[i]} stroke="#FDC550" strokeWidth="4" />
                <path key={`static-bot-${i}`} d={bottomRow[i]} stroke="#FDC550" strokeWidth="4" />
              </>
            ) : (
              <>
                <motion.path
